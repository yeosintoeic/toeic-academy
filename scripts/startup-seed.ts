...
async function main() {
  ...
  for (const u of TARGET_USERS) {
    ...
    if (result.count > 0) {
      console.log(`[startup-seed] ${u.email} → ${u.role} 설정 완료`);
    }
  }
  ...
  if (existing === 0) {
    ...
    console.log(`[startup-seed] 숙제 1번 문제 ${HOMEWORK1_QUESTIONS.length}개 삽입 완료`);
  } else {
    console.log(`[startup-seed] 숙제 1번 문제 이미 존재 (${existing}개), 스킵`);
  }

  console.log("[startup-seed] 완료");
}
